  ...
  while (true) {
    ...
    switch(event) {
      case network_layer_ready: /* the network layer has a packet to send */
        ...
        send_data(next_frame_to_send, frame_expected,_buffer); /* transmit the frame */
      ...
      case timeout: /* trouble; retransmit all outstanding frames */
        next_frame_to send = ack_expected; /* start retransmitting here */
        for (i = 1; i <= nbuffered; i++) {
          send data(next_frame_to_send, frame_expected, buffer);/* resend frame */
